...
int main(){
    ...
    A=rand()%10+5;
    printf("Today %d groups of vistors check in:\n",A);//5-14 vistors per day;
    getchar();
    for(j=0;j<A;j++)
    ...
    printf("Options to add breakfast,change nights(NOT AVAILABLE FOR DORM)press 1 to enter/ANY OTHER NUMBER TO SKIP;)\n");
     ...
     while(f==1){
         ...
         {printf("FAIL TO OPERATE:PLEASE DOUBLE CHECK WHETHER THE ID OF VISTORS IS CORRECT FOR THIS ROOM.\n");}
         else
